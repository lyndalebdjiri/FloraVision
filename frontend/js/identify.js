  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        backendMessage = errorPayload.error || errorPayload.detail || '';
      } catch (_) {
        backendMessage = '';
      }
    ...
    if (err.name === 'AbortError') {
      showError('Prediction timed out after 30 seconds. Please try a smaller or clearer photo.');
    } else {
...
function renderResult(data) {
  ...
  if (data.warning) {
    factText.textContent = data.warning;
    factEl.style.display = 'flex';
  } else if (data.fun_fact) {
